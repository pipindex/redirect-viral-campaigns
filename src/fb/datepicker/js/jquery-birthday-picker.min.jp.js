$(function(e){
	var t={
		...
		"short":["１月","２月","３月","４月","５月","６月","７月","８月","９月","１０月","１１月","１２月"],
		...
		generateBirthdayPicker=function(n,s){
			...
			if(s.placeholder){
				e("<option value='0'>年</option>").appendTo(u);
				e("<option value='0'>月</option>").appendTo(a);
				e("<option value='0'>日</option>").appendTo(f)}
				...
				if(s.defaultDate){
					...
					u.val(d.getFullYear());
					a.val(d.getMonth()+1);
					f.val(d.getDate())}o.append($birthday);
					n.append(o);
					o.on("change",function(){
						selectedYear=parseInt(u.val(),10),
						selectedMonth=parseInt(a.val(),10),
						selectedDay=parseInt(f.val(),10);
			...
			e.fn.birthdayPicker.defaults={
				maxAge:100,minAge:0,maxYear:r,dateFormat:"bigEndian",monthFormat:"number",placeholder:true,defaultDate:false,sizeClass:"span2"}
		}(jQuery))